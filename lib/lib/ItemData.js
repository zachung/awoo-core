...
var ItemData = /*#__PURE__*/function (_Array) {
  ...
  _createClass(ItemData, [{
    ...
    value: function toJson(item) {
      var key = item.type || item.id ? "".concat(item.type, ":").concat(item.id) : '';
      return new ItemData([key, item.chunk ? item.chunk.chunkName : item.chunkName, item.x, item.y, item.props]);
    }
    /**
     * @param {string} key
