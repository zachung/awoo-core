...
var Chunk = /*#__PURE__*/function () {
  ...
  _createClass(Chunk, [{
    ...
    key: "reloadWorld",
    value: function reloadWorld(reader, chunkData) {
      var _this2 = this;

      reader.fromData(chunkData, function (layer, item) {
        if (layer === 'grounds') {
          item.chunk = _this2;

          _this2.groundLayer.remove(item.x, item.y);

          _this2.groundLayer.put(item, item.x, item.y);
        ...
        _this2.itemLayer.remove(item.x, item.y);

        _this2.itemLayer.put(item, item.x, item.y);
      }).then(function () {
        // reload 不需要關注
        _this2.itemLayer.isDirty = false;
      });
    }
  }, {
    key: "getItem",
